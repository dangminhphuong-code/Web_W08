...
// helper: đồng bộ sequence sau khi insert id thủ công
async function syncSerial(trx, table, idCol, schema = 'public') {
  const rel = `${schema}.${quoteIdent(table)}`;
  // Lấy tên sequence từ serial/identity của cột
...
export async function seed(knex) {
  await knex.transaction(async (trx) => {
    ...
    const products = productsRaw.map((p) => ({
      ...
      rating_rate: p.rating?.rate ?? null,
      rating_count: p.rating?.count ?? null,
      category_id: p.category_id
    }));
    await trx('products').insert(products);

    // Đồng bộ sequence cho cả 2 bảng (vì dùng increments)
